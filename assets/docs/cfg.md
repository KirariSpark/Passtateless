...
生成算法由一段「生成脚本」描述。脚本必须按顺序包含两个代码块：

- `GroupInput { … }`：声明生成时需要的外部输入变量。
- `Generate { … }`：声明计算步骤，最终通过 `return` 返回一个字符串（即最终密码）；可以有多个 `return`，运行时首个被执行的生效，若执行完都没有 `return` 则报错。
...
- **条件**：`if(cond: …, onTrue: …, onFalse: …);`（分支可直接 `return 表达式`，命中即结束脚本并作为结果）
- **主动报错**：`raise("错误信息");`
- **占位**：`pass;`
- **返回**：`return 表达式;`

`Generate` 中可以有任意多个 `return`，且 `return` 只能返回字符串；运行时首个被执行的 `return` 生效，若执行完整个 Generate 都没有执行到 `return` 则运行时报错。
...
- `cond`：一个布尔表达式。
- `onTrue` / `onFalse`：分别表示条件为真 / 为假时执行的分支，可以是普通表达式、`pass`（不产生结果）、`raise("…")`（抛出错误）或 `return 表达式`（立即结束脚本并以其值作为结果）。两者都省略时默认视为 `pass`。
...
- **`syntax`**：语法不合法、输入变量重复声明、未声明 `master`/`seedString` 等。
